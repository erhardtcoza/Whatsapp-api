...
export default {
  async fetch(request, env) {
    ...
    if (path === "/webhook" && request.method === "POST") {
      ...
      const hol = await env.DB.prepare(
        "SELECT 1 FROM public_holidays WHERE date=?"
      ).bind(today).first();
      if (hol) {
        const reply = "Closed today for a public holiday.";
        await sendWhatsAppMessage(from, reply, env);
        await env.DB.prepare(
          `INSERT INTO messages (from_number, body, tag, timestamp, direction)
           VALUES (?, ?, 'system', ?, 'outgoing')`
      ...
      if (type === "text") {
        ...
        if (msgObj.audio?.voice) {
          const ar = "Cannot process voice notes. Please send text.";
      ...
      if (customer && customer.verified === 1) {
        ...
        if (dept) {
          ...
          const { count } = await env.DB.prepare(
            `SELECT COUNT(*) AS count FROM chatsessions WHERE ticket LIKE ?`
          ).bind(`${dateKey}-%`).first();
          const seq = (count||0)+1;
        ...
        // routed message under existing session
        await env.DB.prepare(
      ...
      // UNVERIFIED → registration prompt
      const prompt =
    ...
    // — Send Message page: list customers —
    ...
    if (path === "/api/close-chat" && request.method === "POST") {
      ...
      const note = "This chat has been closed. Reply 'hi' to start again.";
    ...
    if (path === "/api/flow-step" && request.method === "POST") {
      ...
      if (!flow_id||!prompt) return new Response("Missing fields", { status:400 });
      if (id) {
        await env.DB.prepare(
          `UPDATE flow_steps
           SET step_order=?, prompt=?, response=?
           WHERE id=?`
        ).bind(step_order||0,prompt,response||"",id).run();
      } else {
        await env.DB.prepare(
          `INSERT INTO flow_steps (flow_id,step_order,prompt,response)
           VALUES(?,?,?,?)`
