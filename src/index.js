...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/webhook" && request.method === "POST") {
      ...
      // your own customer table lookup
    ...
    // --- DEPARTMENTAL CHAT LISTS (latest message per phone) ---
    ...
    if (url.pathname === "/api/unlinked-clients" && request.method === "GET") {
      ...
      `;
      const { results } = await env.DB.prepare(sql).all().catch(_=>({results:[]}));
      return withCORS(Response.json(results));
    }
    ...
    if (url.pathname === "/api/close-session" && request.method === "POST") {
      ...
      ).bind(Date.now(), "admin", ticket).run();
      return withCORS(Response.json({ ok: true }));
    }
