...
export default {
  async fetch(request, env, ctx) {
    ...
    // --- API: GET chat-sessions for a phone ---
    ...
    // --- API: Broadcasts CRUD ---
    // Assumes a `broadcasts` table: (id INTEGER PRIMARY KEY, message TEXT, scheduled_ts INTEGER)
    if (url.pathname === "/api/broadcasts" && request.method === "GET") {
      const { results } = await env.DB.prepare(
        `SELECT id, message, scheduled_ts FROM broadcasts ORDER BY scheduled_ts DESC`
      ).all();
      return withCORS(Response.json(results));
    }
    if (url.pathname === "/api/broadcast" && request.method === "POST") {
      const { id, message, scheduled_ts } = await request.json();
      if (!message || !scheduled_ts) {
        return withCORS(new Response("Missing fields", { status: 400 }));
      }
      if (id) {
        await env.DB.prepare(
          `UPDATE broadcasts SET message=?, scheduled_ts=? WHERE id=?`
        ).bind(message, scheduled_ts, id).run();
      } else {
        await env.DB.prepare(
          `INSERT INTO broadcasts (message, scheduled_ts) VALUES (?, ?)`
        ).bind(message, scheduled_ts).run();
      }
      return withCORS(Response.json({ ok: true }));
    }
    if (url.pathname === "/api/broadcast-delete" && request.method === "POST") {
      const { id } = await request.json();
      if (!id) return withCORS(new Response("Missing broadcast id", { status: 400 }));
