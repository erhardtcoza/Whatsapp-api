  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/webhook" && request.method === "POST") {
      ...
      if (msgObj.type === "text") {
        ...
        if (msgObj.audio?.voice) {
          const autoReply =
            "Sorry, but we cannot receive voice notes. Please send text or documents.";
    ...
    if (url.pathname === "/api/close-chat" && request.method === "POST") {
      ...
      await env.DB.prepare(`UPDATE messages SET closed = 1 WHERE from_number = ?`)
        ...
        .run();
      const notice =
        "This chat has been closed. To start a new session, just say hi.";
    ...
    if (url.pathname === "/api/users" && request.method === "GET") {
      const { results } = await env.DB
        .prepare(`SELECT id, username, role FROM admins ORDER BY username`)
        .all();
      return withCORS(Response.json(results));
    }
    if (url.pathname === "/api/add-user" && request.method === "POST") {
      ...
      if (!username || !password || !role)
        return withCORS(new Response("Missing fields", { status: 400 }));
      await env.DB
        .prepare(`INSERT INTO admins (username, password, role) VALUES (?, ?, ?)`)
    ...
    if (url.pathname === "/api/office-global" && request.method === "POST") {
      ...
      await env.DB
        .prepare(`UPDATE office_global SET closed = ?, message = ? WHERE id = 1`)
    ...
    if (url.pathname === "/api/flows" && request.method === "GET") {
      const { results } = await env.DB.prepare(`SELECT id, name FROM flows ORDER BY id`).all();
      return withCORS(Response.json(results));
    }
    // Create flow
    if (url.pathname === "/api/flows" && request.method === "POST") {
      ...
      const insert = await env.DB.prepare(`INSERT INTO flows (name) VALUES (?)`).bind(name).run();
      return withCORS(Response.json({ id: insert.lastInsertRowid, name }));
    }
    // Delete flow (and its steps)
    if (url.pathname.match(/^\/api\/flows\/\d+$/) && request.method === "DELETE") {
    ...
    if (url.pathname.match(/^\/api\/flows\/\d+\/steps$/) && request.method === "POST") {
      ...
      for (const step of newSteps) {
        await env.DB.prepare(`
          ...
          VALUES (?, ?, ?)
        `).bind(flowId, step.condition, step.response).run();
      }
      return withCORS(Response.json({ ok: true }));
    }
